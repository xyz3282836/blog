import{_ as t}from"./plugin-vue_export-helper-x3n3nnut.js";import{r as l,o as d,c,b as n,e as s,d as i,a as e}from"./app-88a2Q8n1.js";const o="/assets/image-20220709144517363-oJd_EZwZ.png",u={},r=n("h2",{id:"基本指令",tabindex:"-1"},[n("a",{class:"header-anchor",href:"#基本指令","aria-hidden":"true"},"#"),s(" 基本指令")],-1),p={href:"https://go.xargin.com/docs/assembly/assembly/",target:"_blank",rel:"noopener noreferrer"},v=e(`<ul><li>MOV 数据搬运，赋值</li><li>LEA 取址 &amp;</li><li>TEST</li><li>CMP 比较</li><li>JLS （ jump less ）小于时跳转</li><li>JMP（ jump ）无条件跳转</li><li>JEQ（ jump equal ）等于时跳转</li><li>SUB 相减，结果保存到目标操作数</li><li>ADD 相加，结果保存到目标操作数</li><li>RET 返回</li><li>NOP 空指令</li><li>CALL 调用函数</li><li>SHL（ shift logical left ） 逻辑左移指令 <code>&lt;&lt;</code></li><li>SHR（ shift logical right ） 逻辑右移指令 <code>&gt;&gt;</code>，右移时不保留操作数的符号，用 0 代替</li><li>SAL（ shift arithmetic left ）算术左移指令</li><li>SAR（ shift arithmetic right ）算术右移指令，右移时保留操作数的符号</li><li>XADD：交换并相加</li><li>NEG：求补指令，就是取相反数</li><li>XORPS：源操作数（第二个操作数）与目标操作数（第一个操作数）进行异或。结果保存到目标操作数</li><li>MOVUPS：与 MOV 一样，操作对象的类型不一样，这里是包含四个压缩单精度浮点值的双四字</li><li>XCHG: 交换两个操作数内容，自带 LOCK 总线锁属性</li></ul><blockquote><p>MOV 指令有有好几种后缀 MOVB MOVW MOVL MOVQ 分别对应的是 1 字节 、2 字节 、4 字节、8 字节</p></blockquote><h3 id="栈调整" tabindex="-1"><a class="header-anchor" href="#栈调整" aria-hidden="true">#</a> 栈调整</h3><p>intel 或 AT&amp;T 汇编提供了 push 和 pop 指令族，<s>plan9 中没有 push 和 pop</s>，plan9 中虽然有 push 和 pop 指令，但一般生成的代码中是没有的，我们看到的栈的调整大多是通过对硬件 SP 寄存器进行运算来实现的，例如:</p><div class="language-asm" data-ext="asm"><pre class="language-asm"><code>SUBQ $0x18, SP // 对 SP 做减法，为函数分配函数栈帧
...               // 省略无用代码
ADDQ $0x18, SP // 对 SP 做加法，清除函数栈帧
</code></pre></div><p>通用的指令和 X64 平台差不多，下面分节详述。</p><p>常数在 plan9 汇编用 $num 表示，可以为负数，默认情况下为十进制。可以用 $0x123 的形式来表示十六进制数。</p><h3 id="数据计算" tabindex="-1"><a class="header-anchor" href="#数据计算" aria-hidden="true">#</a> 数据计算</h3><p>data moves from left to right</p><div class="language-asm" data-ext="asm"><pre class="language-asm"><code>ADD R1, R2 // R2 += R1
SUB R3, R4, R5 // R5 = R4 - R3
MUL $7, R6 // R6 *= 7
</code></pre></div><h3 id="内存操作" tabindex="-1"><a class="header-anchor" href="#内存操作" aria-hidden="true">#</a> 内存操作</h3><p>memory oprands: offset + reg1 + reg2*scale</p><p><code>(AX)</code>表示 <code>*AX</code>，取地址对应的值</p><div class="language-asm" data-ext="asm"><pre class="language-asm"><code>MOV (R1), R2 // R2 = *R1  取R1的地址对应的值
MOV 8(R3), R4 // R4 = *(8 + R3)
...
</code></pre></div><h3 id="条件调转" tabindex="-1"><a class="header-anchor" href="#条件调转" aria-hidden="true">#</a> 条件调转</h3><div class="language-asm" data-ext="asm"><pre class="language-asm"><code>// 跳转
JMP addr
JMP label
JMP 2(PC)
JMP -2(PC)
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>asm</p><div class="language-asm" data-ext="asm"><pre class="language-asm"><code>0x001d 00029 (arr_par.go:10)    MOVQ    &quot;&quot;.statictmp_0(SB), AX
...
</code></pre></div><p>编译器在某些情况下已经考虑到了性能问题，帮助我们使用 SIMD 指令集来对数据搬运进行了优化。</p><h3 id="demo" tabindex="-1"><a class="header-anchor" href="#demo" aria-hidden="true">#</a> demo</h3><p>main.go</p><div class="language-go line-numbers-mode" data-ext="go"><pre class="language-go"><code><span class="token keyword">package</span> main
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><div class="language-bash" data-ext="sh"><pre class="language-bash"><code>go tool compile <span class="token parameter variable">-N</span> <span class="token parameter variable">-l</span> <span class="token parameter variable">-S</span> main.go <span class="token operator">&gt;</span> main.tool.compile
</code></pre></div><p>main.tool.compile</p><div class="language-asm line-numbers-mode" data-ext="asm"><pre class="language-asm"><code>&quot;&quot;.addInt STEXT size=177 args=0x18 locals=0x20 funcid=0x0
